...
export function WechatOneClickPanel({
  ...
  if (!isAvailable) {
    return (
      <div className="space-y-4">
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-gold-500/25 bg-gold-500/10 px-5 py-8 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gold-500/15">
            <AlertCircle className="text-gold-600" size={22} />
          </div>
          <div>
            <p className="text-sm font-medium text-gold-600">一键登录暂未启用</p>
            <p className="mt-1 max-w-sm text-xs text-ink-500 leading-relaxed">
              后端尚未配置微信开放平台 AppID，可以改用 <button onClick={onSwitchToQrcode} className="underline underline-offset-2 hover:text-gold-600">扫码登录</button>。
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-2 rounded-2xl border border-ink-700/10 bg-rice-100 p-3 text-[11px] text-ink-500">
        <div className="flex items-start gap-2">
          <span className="mt-0.5 text-celadon-600">●</span>
          <span>
            {inWechat
...
function PrimaryButton({ phase, onClick, inWechat, mode }) {
  ...
  const Icon = phase === 'starting' || phase === 'waiting' ? Loader2 : phase === 'success' ? CheckCircle2 : null
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'bg-[#07C160]',
        'shadow-[0_10px_28px_rgba(7,193,96,0.25)] hover:brightness-105 active:scale-[0.98]',
    ...
    >
      ...
      >
        {Icon ? <Icon size={16} className={phase === 'starting' || phase === 'waiting' ? 'animate-spin' : ''} /> : <span className="text-sm font-bold">微</span>}
      </span>
      <span>{label}</span>
      {mode === 'h5' && (
