...
普通用户不使用邮箱密码登录。普通用户通过微信小程序扫码登录 Web，并且 Web 账号和小程序天然绑定。管理员后台单独使用账号密码登录。
...
session_key：微信返回的会话密钥，只后端短暂使用，不返回前端
```
...
3. 如果都找不到，首次扫码登录时自动创建 Chatty 用户。
...
用户在 Web 登录页点击“微信扫码登录”。

完整流程：

1. Web 调用后端创建登录会话。
2. 后端生成一个 `loginSessionId`。
3. 后端生成一个 `scene`，用于小程序码。
4. 后端调用微信接口生成小程序码图片。
5. Web 展示小程序码。
6. 用户用微信扫码打开 Chatty 小程序。
7. 小程序调用 `wx.login` 获取 `code`。
8. 小程序把 `code` 发给后端。
9. 后端调用微信 `code2Session`，拿到 `openid/unionid`。
10. 小程序展示登录确认页。
11. 用户点击“确认登录”。
12. 后端将登录会话标记为 confirmed。
13. Web 轮询到 confirmed。
14. 后端给 Web 写入登录 Cookie。
15. Web 进入 Chatty 聊天页面。
